...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if set(action_node.prenodes).issubset(self.s_levels[level]):
                valid_actions.add(action_node)
                for s_node in action_node.prenodes:
                    action_node.parents.add(s_node)
                    s_node.children.add(action_node)
        self.a_levels.append(valid_actions)
    ...
    def add_literal_level(self, level):
        ...
        for action in self.a_levels[level - 1]:
            for literal in action.effnodes:
                valid_literals.add(literal)
                literal.parents.add(action)
                action.children.add(literal)

        self.s_levels.append(valid_literals)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        effects_action_1 = node_a1.effnodes
        effects_action_2 = node_a2.effnodes

        for literal in effects_action_1:
            for literal_other in effects_action_2:
                if literal.symbol == literal_other.symbol and literal.is_pos != literal_other.is_pos:
                    return True
        return False

        return set(eff_action_1).issubset(eff_action_2)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        precond_action_1 = node_a1.prenodes
        precond_action_2 = node_a2.prenodes
        effects_action_1 = node_a1.effnodes
        effects_action_2 = node_a2.effnodes

        for literal_effect in effects_action_1:
            for literal_precond in precond_action_2:
                if literal_effect.symbol == literal_precond.symbol and literal_effect.is_pos != literal_precond.is_pos:
                    return True
        for literal_effect in effects_action_2:
            for literal_precond in precond_action_1:
                if literal_effect.symbol == literal_precond.symbol and literal_effect.is_pos != literal_precond.is_pos:
                    return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        parents_action_1 = node_a1.parents
        parents_action_2 = node_a2.parents

        for literal in parents_action_1:
            for literal_other in parents_action_2:
                if literal.is_mutex(literal_other) or literal_other.is_mutex(literal):
                    return True
        return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        if node_s1.symbol == node_s2.symbol and node_s1.is_pos !=node_s2:
            return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for parent in parents_1:
            for parent_other in parents_2:
                if not parent.is_mutex(parent_other) and not parent_other.is_mutex(parent):
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in goals:
            for level_index, level_states in enumerate(self.s_levels):
                for state_literal in level_states:
                    if goal == state_literal.symbol:
